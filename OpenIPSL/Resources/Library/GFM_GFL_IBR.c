...
__declspec(dllexport) void* __cdecl init_gfm_model(double c_filt, double r_filt, double l_filt, double r_l,
                                                    double Vbase,double Sbase,double VDC_base,double w_type)
{
    ...
    model_parameters->KfDroop = 30;
    model_parameters->KvDroop = 22.22;
    ...
    model_parameters->Rchoke = r_l;
    model_parameters->Lchoke = l_filt;
    model_parameters->Cfilt = c_filt;
    model_parameters->Rdamp = r_filt;
...
__declspec(dllexport) void __cdecl deinit_gfm_model(IEEE_Cigre_DLLInterface_Instance* instance)
{
...
// ----------------------------------------------------------------
__declspec(dllexport) double* __cdecl model_calculate(IEEE_Cigre_DLLInterface_Instance* instance, double* out) {
    MyModelOutputs* outputs = (MyModelOutputs*)instance->ExternalOutputs;
    ...
    out[0] = outputs->Ea * 1000;
    out[1] = outputs->Eb * 1000;
    out[2] = outputs->Ec * 1000;
    out[3] = outputs->Idrefout;
    out[4] = outputs->Idout;
    out[5] = outputs->Iqrefout;
    out[6] = outputs->Iqout;
    out[7] = outputs->Vdout;
    out[8] = outputs->Vqout;
    out[9] = outputs->Freqpll;
    out[10] = outputs->Pout;
    out[11] = outputs->Qout;
...
__declspec(dllexport) void __cdecl save_states(IEEE_Cigre_DLLInterface_Instance* instance)
{
    MyModelOutputs* outputs = (MyModelOutputs*)instance->ExternalOutputs;
    MyModelInputs* inputs = (MyModelInputs*)instance->ExternalInputs;

    int listSize = 12+12+36;
    ...
    if (file != NULL)
    {
        ...
        list[13] = outputs->Eb;
        list[14] = outputs->Ec;
        list[15] = outputs->Idrefout;
        list[16] = outputs->Idout;
        list[17] = outputs->Iqrefout;
        list[18] = outputs->Iqout;
        list[19] = outputs->Vdout;
        list[20] = outputs->Vqout;
        list[21] = outputs->Freqpll;
        list[22] = outputs->Pout;
        list[23] = outputs->Qout;

        for (int i = 0; i < 36; i++)
        {
            list[24+i] = instance->DoubleStates[i];
        }
        fwrite(list, sizeof(double), listSize, file);
        fclose(file);
    }
}

__declspec(dllexport) double __cdecl load_states(IEEE_Cigre_DLLInterface_Instance* instance)
{
    MyModelOutputs* outputs = (MyModelOutputs*)instance->ExternalOutputs;
    MyModelInputs* inputs = (MyModelInputs*)instance->ExternalInputs;
    ...
    FILE* file = fopen("list.dat", "rb");
    if (file != NULL)
    {
        fread(list, sizeof(double), listSize, file);
        fclose(file);
        inputs->Va = list[0];
        inputs->Vb = list[1];
        inputs->Vc = list[2];
        ...
        outputs->Eb = list[13];
        outputs->Ec = list[14];
        outputs->Idrefout = list[15];
        outputs->Idout = list[16];
        outputs->Iqrefout = list[17];
        outputs->Iqout = list[18];
        outputs->Vdout = list[19];
        outputs->Vqout = list[20];
        outputs->Freqpll = list[21];
        outputs->Pout = list[22];
        outputs->Qout = list[23];
        for (int i = 0; i < 36; i++)
        {
            instance->DoubleStates[i] = list[24 + i];
        }
        return list[12];
    }
    return 0;
...
__declspec(dllexport) void __cdecl free_out(double* out) {
    free(out);
}

__declspec(dllexport) double __cdecl my_add(double a, double b)
{
    return a + b;
}

__declspec(dllexport) double __cdecl get_sim_time(void)
{
    return sim_time += 1;
}
